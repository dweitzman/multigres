...
//
//  1. Consensus participation: the node will refuse BeginTerm or other coordinator
...
// AvailabilityStatus contains operational fitness signals for a node.
// These inform coordinator decisions about cohort membership, primary preference,
// and routing — separate from protocol position (ConsensusStatus).
//
// Signals here are best-effort: they are held in memory and may be absent after
// a process restart. Coordinators must treat absence as "unknown," not "fit."
// Applies to both primaries and standbys.
type AvailabilityStatus struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Non-zero when this node has voluntarily resigned its primary role at the given term.
	// The coordinator should initiate a new election rather than waiting for timeout detection.
	// Cleared when the node observes a newer rule with a different primary.
...
// system: its term revocation, what it has committed, and what it knows is coming.
...
var file_clustermetadata_proto_depIdxs = []int32{
	...
	27, // 25: clustermetadata.TermRevocation.coordinator_initiated_at:type_name -> google.protobuf.Timestamp
