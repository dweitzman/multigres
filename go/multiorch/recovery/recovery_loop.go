...
func (re *Engine) processShardProblems(ctx context.Context, shardKey commontypes.ShardKey, problems []types.Problem) {
	ctx, span := telemetry.Tracer().Start(ctx, "recovery/shard",
		trace.WithAttributes(
			attribute.String("database", shardKey.Database),
			attribute.String("tablegroup", shardKey.TableGroup),
			attribute.String("shard", shardKey.Shard),
			attribute.Int("problem_count", len(problems)),
		))
	defer span.End()
...
func (re *Engine) attemptRecovery(ctx context.Context, problem types.Problem) {
	...
	actionCtx, cancel := context.WithTimeout(ctx, problem.RecoveryAction.Metadata().Timeout)
	defer cancel()

	startTime := time.Now()

	err = problem.RecoveryAction.Execute(actionCtx, problem)
...
func (re *Engine) recheckProblem(ctx context.Context, problem types.Problem) (bool, error) {
	...
	ctx, span := telemetry.Tracer().Start(ctx, "recovery/recheck")
	defer span.End()
	...
	recheckCtx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()

	// Refresh metadata for the shard
	if err := re.refreshShardMetadata(recheckCtx, problem.ShardKey, nil); err != nil {
	...
	if isShardWide {
		...
		re.forceHealthCheckShardPoolers(recheckCtx, problem.ShardKey, poolersToIgnore)
	} else {
		...
		if ph, ok := re.poolerStore.Get(poolerIDStr); ok {
			re.pollPooler(recheckCtx, ph.MultiPooler.Id, ph, true /* forceDiscovery */)
		}
