...
func (pm *MultiPoolerManager) takeRemedialAction(ctx context.Context, action remedialAction) {
	...
	case remedialActionReopenManager:
		...
		if err := pm.Open(); err != nil {
			...
			pm.pgMonitorMetrics.remedialActionDuration.Record(ctx, time.Since(startTime).Seconds(), RemedialActionReopenManager, RemedialActionStatusFailure)
			return
		}
		pm.logger.InfoContext(ctx, "MonitorPostgres: successfully reopened manager")
		pm.pgMonitorMetrics.remedialActionDuration.Record(ctx, time.Since(startTime).Seconds(), RemedialActionReopenManager, RemedialActionStatusSuccess)
		return
