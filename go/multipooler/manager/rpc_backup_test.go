...
`
			...
			err := exec.CommandContext(ctx, "sh", "-c", "cat > "+pgbackrestPath+" << 'EOF'\n"+mockScript+"\nEOF").Run()
			require.NoError(t, err)
			err = exec.CommandContext(ctx, "chmod", "+x", pgbackrestPath).Run()
			require.NoError(t, err)
...
func BenchmarkSafeCombinedOutput(b *testing.B) {
	b.Run("Small output", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			cmd := exec.CommandContext(context.TODO(), "echo", "hello world")
	...
	b.Run("Medium output (100 lines)", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			cmd := exec.CommandContext(context.TODO(), "sh", "-c", "for i in $(seq 1 100); do echo \"Line $i\"; done")
			_, _ = safeCombinedOutput(cmd)
		}
	})
	...
	b.Run("Large output (1000 lines)", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			cmd := exec.CommandContext(context.TODO(), "sh", "-c", "for i in $(seq 1 1000); do echo \"Line $i\"; done")
			_, _ = safeCombinedOutput(cmd)
		}
	})
	...
	b.Run("Mixed stdout and stderr", func(b *testing.B) {
		...
		for i := 0; i < b.N; i++ {
			cmd := exec.CommandContext(context.TODO(), "sh", "-c", script)
			_, _ = safeCombinedOutput(cmd)
		}
	})
}
