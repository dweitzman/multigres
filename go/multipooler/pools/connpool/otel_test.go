...
func TestOTelConnectionCount_IdleTimeout(t *testing.T) {
	setup := telemetry.SetupTestTelemetry(t)
	...
	// Very short idle timeout to trigger expiration quickly
	pool := NewPool[*mockConnection](ctx, &Config{
		Capacity:        5,
		MaxIdleCount:    5,
	...
	sum := getConnectionCountMetric(t, setup.MetricReader)
	assert.Equal(t, int64(0), getStateCount(sum, "idle-timeout-pool", "used"), "used should be 0")
	assert.Equal(t, int64(1), getStateCount(sum, "idle-timeout-pool", "idle"), "idle should be 1")

	// Wait for idle timeout to expire and background worker to close the connection.
	// The background worker runs every IdleTimeout/10 = 5ms, and we need to wait
	// for the connection to be both expired AND popped from the stack.
	require.Eventually(t, func() bool {
		// Get a new connection - this will pop the expired one and create a new one
		conn2, err := pool.Get(ctx)
		if err != nil {
			return false
		}
		...
		// After the expired connection is popped and discarded, and a new one created,
		// idle should still be 1 (the new connection)
		sum = getConnectionCountMetric(t, setup.MetricReader)
		idle := getStateCount(sum, "idle-timeout-pool", "idle")
		used := getStateCount(sum, "idle-timeout-pool", "used")
		// Metrics should be balanced: used=0, idle=1 (the new connection)
		return used == 0 && idle == 1
	}, time.Second, 10*time.Millisecond, "metrics should remain balanced after idle timeout")
}
