...
// PoolerHashProvider implements auth.PasswordHashProvider by fetching
...
// Returns auth.ErrUserNotFound if the user does not exist or has no password.
func (p *PoolerHashProvider) GetPasswordHash(ctx context.Context, username, database string) (*auth.ScramHash, error) {
	...
	hash, err := auth.ParseScramSHA256Hash(resp.ScramHash)
	if err != nil {
...
// Ensure PoolerHashProvider implements auth.PasswordHashProvider
var _ auth.PasswordHashProvider = (*PoolerHashProvider)(nil)
