...
func (pd *CellPoolerDiscovery) processPoolerChange(watchData *topoclient.WatchDataRecursive) {
	...
	// Handle deletion: nil contents for a pooler path
	if watchData.Contents == nil && strings.HasSuffix(watchData.Path, "/Pooler") {
		// Extract pooler name from path (e.g., "poolers/my-pooler/Pooler" -> "my-pooler")
		parts := strings.Split(watchData.Path, "/")
		if len(parts) >= 2 {
			poolerName := parts[len(parts)-2]
			poolerID := pd.cell + "/" + poolerName
			if oldPooler, exists := pd.poolers[poolerID]; exists {
				delete(pd.poolers, poolerID)
				pd.lastRefresh = time.Now()
				pd.logger.Info("Pooler removed", "id", poolerID)
				if pd.onPoolerRemoved != nil {
					pd.onPoolerRemoved(oldPooler.MultiPooler)
				}
			}
