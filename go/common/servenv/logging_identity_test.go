...
func TestServiceIdentityInLogs(t *testing.T) {
	for _, tc := range []struct {
		...
		{"pooler", ServiceIdentity{ServiceName: "multipooler", ServiceInstanceID: "dqxscvcb", Cell: "zone2", Shard: "0-inf", TableGroup: "default"}, map[string]string{"service.instance.id": "zone2-dqxscvcb", "cloud.availability_zone": "zone2", "multigres.shard": "0-inf", "multigres.tablegroup": "default"}},
		{"gateway", ServiceIdentity{ServiceName: "multigateway", ServiceInstanceID: "gw1", Cell: "zone1"}, map[string]string{"service.instance.id": "zone1-gw1", "cloud.availability_zone": "zone1"}},
		...
		for _, format := range []string{"json", "text"} {
			t.Run(tc.name+"/"+format, func(t *testing.T) {
				...
				keys := []string{"service.instance.id", "cloud.availability_zone", "multigres.shard", "multigres.tablegroup"}
