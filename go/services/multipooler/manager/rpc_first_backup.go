...
//
// Returns (busy=true, nil) if the backup lease is held by another pooler — the monitor
// should back off and retry. Returns (false, err) on any other failure.
func (pm *MultiPoolerManager) createFirstBackupAndInitialize(ctx context.Context) (busy bool, retErr error) {
	...
	if err != nil {
		...
		return false, mterrors.Wrap(err, "failed to acquire backup lease")
	}
	defer unlock(&retErr)
	...
	if pm.hasCompleteBackups(lockCtx) {
		pm.logger.InfoContext(lockCtx, "First backup already exists (created by another pooler), skipping")
		return false, nil
	}

	return false, pm.createFirstBackupLocked(lockCtx)
}
...
func (pm *MultiPoolerManager) createFirstBackupLocked(ctx context.Context) error {
	...
	// Read the durability policy from topology and create the policy record.
