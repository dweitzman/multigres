...
// ----------------------------------------------------------------------------
// Lock tokens and dual-lock invariant
// ----------------------------------------------------------------------------
...
func (rs *ruleStore) readCurrentRule(ctx context.Context, forUpdate bool) (*clustermetadatapb.PoolerPosition, error) {
	...
	if len(result.Rows) == 0 {
		return nil, mterrors.Errorf(mtrpcpb.Code_INTERNAL, "current_rule row missing for shard 0: tables may not be initialized")
	}
...
func (rs *ruleStore) updateRule(ctx context.Context, update *ruleUpdateBuilder) (*clustermetadatapb.PoolerPosition, error) {
	...
	if update.externallyCertified {
		...
		transition, err = consensus.BuildSimpleTransitionPolicy(outgoingPWC, incomingPWC)
		if err != nil {
	...
	if isPromotion {
		if err := rs.syncStandby.SetPolicy(lockedCtx, transition.Both, newLeader); err != nil {
	...
	if err := rs.syncStandby.SetPolicy(lockedCtx, transition.Incoming, newLeader); err != nil {
