...
// currentRuleRecord reads the current rule and WAL LSN from postgres, updates the
// position cache, and returns the observed node position.
// Returns nil if no rule has been applied yet (coordinator_term = 0).
func (pm *MultiPoolerManager) currentRuleRecord(ctx context.Context) (*clustermetadatapb.NodePosition, error) {
	return pm.rules.observePosition(ctx)
}

// updateRule delegates to ruleStore.updateRule. See ruleStore.updateRule for full docs.
func (pm *MultiPoolerManager) updateRule(ctx context.Context, update *ruleUpdateBuilder) (*clustermetadatapb.NodePosition, error) {
