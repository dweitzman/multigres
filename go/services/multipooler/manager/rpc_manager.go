...
func (pm *MultiPoolerManager) emergencyDemoteLocked(ctx context.Context, consensusTerm int64, drainTimeout time.Duration) (*multipoolermanagerdatapb.EmergencyDemoteResponse, error) {
	...
	// Build the consensus status to return. We use the inconsistent variant since
	// we're already holding the action lock but the caller needs the post-demotion
	// view (including the resignation signal) without a second lock acquisition.
	consensusStatus, statusErr := pm.getConsensusStatus(ctx)
	if statusErr != nil {
		pm.logger.WarnContext(ctx, "Failed to build consensus status for demote response", "error", statusErr)
	}
