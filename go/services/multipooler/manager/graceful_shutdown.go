...
// setLifecycleStatus stores the new lifecycle signal and deadline under pm.mu.
// It does NOT broadcast — the caller is responsible for invoking
// healthStreamer.Broadcast (non-terminal) or healthStreamer.Shutdown (terminal,
// at the end of graceful shutdown) immediately after, so the new state reaches
// subscribers without waiting for the next periodic heartbeat.
func (pm *MultiPoolerManager) setLifecycleStatus(signal clustermetadatapb.LifecycleSignal, deadlineAt time.Time) {
	pm.withLock(func() {
		pm.lifecycleSignal = signal
		pm.lifecycleDeadlineAt = deadlineAt
	})
}

// GracefulShutdown drains the pooler, announces shutdown intent to the
// orchestrator, closes the connection pool, stops Postgres via pgctld, and
// announces completion. Wired as a servenv OnTermSync hook so it runs on
// SIGTERM within the lameduck window.
//
// The sequence is:
//
...
//     deadline. Without this we could linger indefinitely after announcing
//     SHUTTING_DOWN, which would block the orchestrator's post-EOF failover
//     and risk staying alive serving writes long after the operator asked
//     us to leave. Operators who want to exit faster than the deadline
//     should send SIGKILL.
//
...
//     reserved connections on grace-period timeout. Drain happens *before*
//     the orchestrator-facing announcement so SHUTTING_DOWN does not appear
//     until in-flight queries have finished.
//
//  3. Set lifecycleSignal=SHUTTING_DOWN and broadcast so the orchestrator
//     receives the announcement and switches the stream-EOF handling for
//     this pooler from the usual reconnect-with-backoff to synthesizing
//     STOPPED on EOF.
//
//  4. Close the connection pool. Idle backends are released immediately;
...
//
//  5. Stop Postgres via pgctld.Stop, escalating fast → immediate. Smart is
...
//
//  6. Set lifecycleSignal=STOPPED and broadcast as a terminal snapshot, so
//     the orchestrator can fail over against a leader that has cleanly
//     resigned.
func (pm *MultiPoolerManager) GracefulShutdown(ctx context.Context) {
	...
	// to leave, which would block the orchestrator's post-EOF failover from
	// completing and risk a split-brain window.
	//
	...
	// reserved on grace-period timeout. Done before the SHUTTING_DOWN
	// announce so subscribers see SHUTTING_DOWN only after in-flight
	...
	// Phase 2: announce. The orchestrator switches its stream-EOF handling
	// for this pooler from reconnect-with-backoff to synthesizing STOPPED on
	// EOF. Not terminal: subscribers continue to receive updates through
	// the slow phases below.
	pm.setLifecycleStatus(
		clustermetadatapb.LifecycleSignal_LIFECYCLE_SIGNAL_SHUTTING_DOWN,
		deadlineAt,
	)
	pm.healthStreamer.Broadcast()

	// Phase 3: close the connection pool. Idle backends are released
	...
	// Phase 4: stop Postgres.
	pm.stopPostgresWithEscalation(ctx)

	// Phase 5: announce completion. STOPPED is the orchestrator's primary
	// trigger to fail over; the EOF on the health stream is a backup trigger
	// in case this snapshot is lost in flight.
	//
	// healthStreamer.Shutdown broadcasts the STOPPED state and then closes
	// every subscriber channel so gRPC stream handlers return cleanly via the
	// channel-close path, releasing servenv's grpcServer.GracefulStop wait
	// without forcing the lameduck-period timeout.
	pm.setLifecycleStatus(
		clustermetadatapb.LifecycleSignal_LIFECYCLE_SIGNAL_STOPPED,
		time.Time{},
	)
	pm.healthStreamer.Shutdown()

	// Phase 6: mark this pooler's topology entry as Type=DRAINED,
