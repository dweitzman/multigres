...
func TestRecordTermPrimary(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.seedRule != nil {
				cs.RecordTermPrimary(tt.seedRule, tt.seedPrimary, nil)
			}
			cs.RecordTermPrimary(tt.callRule, tt.callPrimary, nil)
...
func TestRecordTermPrimary_ReturnsCopies(t *testing.T) {
	cs := NewConsensusState(t.TempDir(), nil)
	cs.RecordTermPrimary(ruleAt(5, 0), primaryAt("p1", "hostA", 5432), nil)
