...
// ssmTestPolicy returns an AtLeastNPolicy that, with ssmTestCohort(), produces:
//
//	synchronous_commit = 'on'
//	synchronous_standby_names = 'ANY 1 ("zone1_standby-1")'
func ssmTestPolicy() commonconsensus.DurabilityPolicy {
	return commonconsensus.AtLeastNPolicy{N: 2}
}
...
func TestSetPolicy_SkipsQueriesWhenCached(t *testing.T) {
	...
	ctx := withRuleRowLock(t.Context())
	err := ssm.SetPolicy(ctx, ssmTestPolicy(), ssmTestCohort(), nil)
	require.NoError(t, err)
...
func TestSetPolicy_AppliesWhenCacheMisses(t *testing.T) {
	...
	ctx := withRuleRowLock(t.Context())
	require.NoError(t, ssm.SetPolicy(ctx, ssmTestPolicy(), ssmTestCohort(), nil))
