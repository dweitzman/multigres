...
func NewMultiPoolerManagerWithTimeout(logger *slog.Logger, multiPooler *clustermetadatapb.MultiPooler, config *Config, loadTimeout time.Duration) (*MultiPoolerManager, error) {
	...
	// Build order: consensusState → rules (no onChange yet) → nodeConsensus →
	// healthStreamer (needs nodeConsensus) → wire onChange callbacks.
	pm.consensusState = newTermRevocation(pm.multipooler.PoolerDir, pm.serviceID)
	...
	pm.nodeConsensus = newNodeConsensusState(pm.consensusState, pm.rules)
	pm.healthStreamer = newHealthStreamer(logger, multiPooler.Id, multiPooler.TableGroup, multiPooler.Shard, pm.nodeConsensus)
	pm.rules.setOnChange(pm.healthStreamer.Broadcast)
	pm.consensusState.SetOnTermChange(pm.healthStreamer.Broadcast)
