...
func (pm *MultiPoolerManager) discoverPostgresState(ctx context.Context) (postgresState, error) {
	...
	if state.postgresRunning {
		var err error
		state.inRecovery, err = pm.isInRecovery(ctx)
		if err != nil {
			...
			// wrong recovery reading would make a healthy but momentarily-unqueryable
			// replica look like a stale primary and trigger a destructive demote
			// (pg_rewind), or flip the published writable bit on a guess. Surface the
			// error so the monitor skips remediation this tick and retries, matching
			// the bootstrap-sentinel handling below.
...
func (pm *MultiPoolerManager) determineRoleAction(role commonconsensus.ConsensusRole, state postgresState) remedialAction {
	...
	if role != commonconsensus.ConsensusRoleLeader && !state.inRecovery {
	...
	if pm.stateManager.hasDrift(state.inRecovery) {
...
func (pm *MultiPoolerManager) determineReplicationSettingsAction(ctx context.Context, state postgresState) remedialAction {
	...
	if !state.inRecovery && pm.consensusMgr.Rules().HasInconsistentGUC(ctx) {
...
func (pm *MultiPoolerManager) takeRemedialAction(ctx context.Context, action remedialAction, state postgresState) {
	...
	case remedialActionReconcileState:
		...
		pm.logger.InfoContext(ctx, "MonitorPostgres: reconciling drifted state", "in_recovery", state.inRecovery)
		if err := pm.stateManager.fixDrift(ctx, state.inRecovery); err != nil {
	...
	case remedialActionReconcileGUC:
		...
		if err := pm.consensusMgr.Rules().ReconcileGUC(ctx, state.inRecovery); err != nil {
