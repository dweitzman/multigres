...
func TestActionLock_MutationMethodsTimeout(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			poolerType: clustermetadatapb.PoolerType_PRIMARY,
			callMethod: func(ctx context.Context) error {
				return manager.UpdateConsensusRule(ctx, multipoolermanagerdatapb.CohortUpdateOperation_COHORT_UPDATE_OPERATION_ADD, []*clustermetadatapb.ID{serviceID}, &clustermetadatapb.RuleNumber{}, nil, nil)
			},
		},
	}
