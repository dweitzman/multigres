...
func TestSetPrimary_StalePrimaryDemotes(t *testing.T) {
	...
	// guardrail. The post-demote sync fence goes through the (fake) rule store's
	// FenceSyncStandby, asserted below, so it issues no SQL here.
	...
	// 5. getStandbyReplayLSN — best-effort LSN read after demote.
	mockQueryService.AddQueryPattern("SELECT pg_last_wal_replay_lsn",
	...
	// Demoting a stale primary must fence synchronous_standby_names so it can't
	// later promote into a fail-open (unacknowledged-write) state.
	assert.True(t, ruleStore.fenceSyncCalled,
		"stale-primary demotion should fence sync standby names via FenceSyncStandby")
