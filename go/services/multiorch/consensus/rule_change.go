...
// eligibilitySignalWeight maps an EligibilitySignal to a tiebreak weight
// (higher deprioritizes). UNKNOWN/ELIGIBLE are both 0 (no preference either
// way, per the proto's backwards-compat convention).
var eligibilitySignalWeight = map[clustermetadatapb.EligibilitySignal]int{
	clustermetadatapb.EligibilitySignal_ELIGIBILITY_SIGNAL_UNKNOWN:    0,
	clustermetadatapb.EligibilitySignal_ELIGIBILITY_SIGNAL_ELIGIBLE:   0,
	clustermetadatapb.EligibilitySignal_ELIGIBILITY_SIGNAL_INELIGIBLE: 1,
}

// poolerHealthStateLess returns a less function for sort.SliceStable that
// orders ConsensusStatus entries by candidate priority. Used in
// Coordinator.runFailover to prefer nodes with lower priority among candidates
// that share the highest LSN.
//
// WAL position is always the primary criterion: a node with worse priority
// still wins if it holds a strictly higher LSN than every other node. This
// tiebreaker only affects the ordering of tied eligible leaders, never the
// outgoing-cohort quorum denominator.
func poolerHealthStateLess(healthByID map[string]*multiorchdatapb.PoolerHealthState) func(a, b *clustermetadatapb.ConsensusStatus) bool {
	// continue_leadership_signal (demonstrably unfit right now) outweighs a mere
	// become_leader_eligibility_signal preference — weighted 2x so it always
	// dominates that preference in the combined sum.
	candidatePriority := func(cs *clustermetadatapb.ConsensusStatus) int {
		av := healthByID[topoclient.ClusterIDString(cs.GetId())].GetAvailabilityStatus()
		return 2*eligibilitySignalWeight[av.GetContinueLeadershipSignal()] + eligibilitySignalWeight[av.GetBecomeLeaderEligibilitySignal()]
	}
	...
	return func(a, b *clustermetadatapb.ConsensusStatus) bool {
		if pa, pb := candidatePriority(a), candidatePriority(b); pa != pb {
			return pa < pb
		}
		...
		// EligibleLeaders set) and on the signals above, so it never trades data
