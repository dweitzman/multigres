...
// FindHealthyPrimary returns the shard's consensus leader if it is present in
// the given pooler set and live-running as PRIMARY. It separates two concerns:
// identifying who the leader is (from consensus state) and confirming that
// leader is healthy (a live Status RPC).
//
// Leader identity comes purely from consensus: the leader named by the
// most-advanced rule across all poolers' ConsensusStatus. A follower adopts its
// leader's rule, so the most-advanced position names the current leader
// regardless of which pooler holds it; the topology Type label is never
// consulted. A stale primary still running as PRIMARY but not named by the
// highest rule is ignored here — consensus resolves leadership by rule, and
// demoting the stale node is a separate analyzer's responsibility.
func (s *PoolerStore) FindHealthyPrimary(
	...
	for _, pooler := range poolers {
		if pooler.MultiPooler != nil && proto.Equal(pooler.MultiPooler.Id, leaderID) {
			leader = pooler
			break
		}
	}
	...
	// Confirm the leader is reachable and still reports itself as the leader in
	// its live consensus status — a node that has since resigned or dropped into
	// recovery must not be treated as the primary. Leadership is judged from
	// consensus, never the PoolerType label.
