...
func TestHealthStream_StreamEOFAfterSHUTTING_DOWN_SynthesizesSTOPPED(t *testing.T) {
	...
	stream.Ch <- makeLifecycleSnapshotForPrimaryTerm(poolerID, clustermetadata.LifecycleSignal_LIFECYCLE_SIGNAL_SHUTTING_DOWN, 5)
...
//
// Once STOPPED arrives (or the stream EOFs), the helper flips to true and
// failover proceeds. This test only covers the "STOPPED arrives normally"
// path; the EOF path is covered by its dedicated test above.
