...
func TestPrioritySorting(t *testing.T) {
	...
	assert.Equal(t, types.ProblemLeaderUnreachable, problems[0].Code)
...
func TestFilterAndPrioritize_ShardWideOnly(t *testing.T) {
	...
	assert.Equal(t, types.ProblemLeaderUnreachable, filtered[0].Code)
...
func TestRecoveryLoop_PostRecoveryRefresh(t *testing.T) {
	...
	engine.recoveryGracePeriodTracker.Observe(types.ProblemLeaderUnreachable, problems[0].EntityID(), problems[0].RecoveryAction, false)
