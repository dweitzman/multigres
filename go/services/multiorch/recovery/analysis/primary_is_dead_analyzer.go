...
func (a *PrimaryIsDeadAnalyzer) Analyze(poolerAnalysis *store.ReplicationAnalysis) (*types.Problem, error) {
	...
	// A primary can signal its own resignation via EmergencyDemote. Treat this as
	// equivalent to a dead primary so a new election starts immediately, rather than
