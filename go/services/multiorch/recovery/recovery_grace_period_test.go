...
func TestRecoveryGracePeriod_InitialDeadlineReset(t *testing.T) {
	...
	before := time.Now()
	tracker.Observe(types.ProblemLeaderUnreachable, "zone1-pooler1", action, true)
	after := time.Now()

	// Verify the deadline was set
	tracker.mu.Lock()
	key := gracePeriodKey{code: types.ProblemLeaderUnreachable, entityID: "zone1-pooler1"}
...
func TestRecoveryGracePeriod_ObserveFreezesDeadline(t *testing.T) {
	...
	tracker.mu.Lock()
	frozenDeadline := tracker.deadlines[gracePeriodKey{code: types.ProblemLeaderUnreachable, entityID: "zone1-pooler1"}]
	tracker.mu.Unlock()
	...
	tracker.Observe(types.ProblemLeaderUnreachable, "zone1-pooler1", action, false)

	tracker.mu.Lock()
	afterUnhealthyDeadline := tracker.deadlines[gracePeriodKey{code: types.ProblemLeaderUnreachable, entityID: "zone1-pooler1"}]
	tracker.mu.Unlock()
	...
	tracker.mu.Lock()
	stillFrozenDeadline := tracker.deadlines[gracePeriodKey{code: types.ProblemLeaderUnreachable, entityID: "zone1-pooler1"}]
	tracker.mu.Unlock()
	...
	tracker.mu.Lock()
	resetDeadline := tracker.deadlines[gracePeriodKey{code: types.ProblemLeaderUnreachable, entityID: "zone1-pooler1"}]
	tracker.mu.Unlock()
...
func TestRecoveryGracePeriod_DeadlineNotExpired(t *testing.T) {
	...
	problem := types.Problem{
		Code:           types.ProblemLeaderUnreachable,
		ShardKey:       shardKey,
	...
	tracker.Observe(types.ProblemLeaderUnreachable, string(commontypes.FormatShardKey(shardKey)), action, true)
...
func TestRecoveryGracePeriod_FirstObserveUnhealthy(t *testing.T) {
	...
	before := time.Now()
	tracker.Observe(types.ProblemLeaderUnreachable, entityID, action, false)
	after := time.Now()

	// Verify deadline was initialized with base + jitter
	tracker.mu.Lock()
	deadline, exists := tracker.deadlines[gracePeriodKey{code: types.ProblemLeaderUnreachable, entityID: entityID}]
	tracker.mu.Unlock()
	...
	tracker.mu.Lock()
	frozenDeadline := tracker.deadlines[gracePeriodKey{code: types.ProblemLeaderUnreachable, entityID: entityID}]
	tracker.mu.Unlock()
...
func TestRecoveryGracePeriod_ConcurrentAccess(t *testing.T) {
	...
	for range 10 {
		go func() {
			for range 100 {
				tracker.Observe(types.ProblemLeaderUnreachable, entityID, action, true)
	...
	tracker.mu.Lock()
	key := gracePeriodKey{code: types.ProblemLeaderUnreachable, entityID: entityID}
	deadline, exists := tracker.deadlines[key]
	tracker.mu.Unlock()
...
func TestRecoveryGracePeriod_ForceExpireAll(t *testing.T) {
	...
	tracker.Observe(types.ProblemLeaderUnreachable, problemA.EntityID(), action, false)
...
func TestRecoveryGracePeriod_DynamicConfigUpdate(t *testing.T) {
	...
	before2 := time.Now()
	newTracker.Observe(types.ProblemLeaderUnreachable, "zone1-pooler1", newAction, true)
	after2 := time.Now()

	newTracker.mu.Lock()
	key2 := gracePeriodKey{code: types.ProblemLeaderUnreachable, entityID: "zone1-pooler1"}
