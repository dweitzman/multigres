...
func TestMain(m *testing.M) {
	...
	go func() {
		...
		defer cancel()
		cleanupSharedTestSetup(cleanupCtx)
		os.Exit(1)
	}()
...
func cleanupSharedTestSetup(ctx context.Context) {
	...
	if sharedTestSetup.StandbyMultipooler != nil {
		sharedTestSetup.StandbyMultipooler.Stop(ctx)
	}
	if sharedTestSetup.PrimaryMultipooler != nil {
		sharedTestSetup.PrimaryMultipooler.Stop(ctx)
	}

	// Stop pgctld instances
	if sharedTestSetup.StandbyPgctld != nil {
		sharedTestSetup.StandbyPgctld.Stop(ctx)
	}
	if sharedTestSetup.PrimaryPgctld != nil {
		sharedTestSetup.PrimaryPgctld.Stop(ctx)
	}
...
func (p *ProcessInstance) stopPostgreSQL(ctx context.Context) {
...
func initializeStandby(t *testing.T, primaryPgctld *ProcessInstance, standbyPgctld *ProcessInstance, standbyMultipooler *ProcessInstance) error {
	...
	standbyPoolerClient, err := endtoend.NewMultiPoolerTestClient(t.Context(), fmt.Sprintf("localhost:%d", standbyMultipooler.GrpcPort))
	if err != nil {
...
func getSharedTestSetup(t *testing.T) *MultipoolerTestSetup {
	...
	setupOnce.Do(func() {
		...
		// Create the cell
		err = ts.CreateCell(t.Context(), cellName, &clustermetadatapb.Cell{
...
func saveGUCs(t *testing.T, client *endtoend.MultiPoolerTestClient, gucNames []string) map[string]string {
	saved := make(map[string]string)
	for _, gucName := range gucNames {
		value, err := queryStringValue(t.Context(), client, fmt.Sprintf("SHOW %s", gucName))
		if err == nil {
			saved[gucName] = value
		}
	}
...
func restoreGUCs(t *testing.T, ctx context.Context, client *endtoend.MultiPoolerTestClient, savedGucs map[string]string, instanceName string) {
	// T's context is cancelled during Cleanup(), so we take a separate Context argument.
	t.Helper()
...
func validateCleanState(ctx context.Context, setup *MultipoolerTestSetup) error {
	if setup == nil {
		return nil
	}
	...
	if setup.PrimaryMultipooler != nil {
		primaryClient, err := endtoend.NewMultiPoolerTestClient(ctx, fmt.Sprintf("localhost:%d", setup.PrimaryMultipooler.GrpcPort))
		if err != nil {
			return fmt.Errorf("failed to connect to primary: %w", err)
		}
		defer primaryClient.Close()

		if err := validateGUCValue(ctx, primaryClient, "primary_conninfo", "", "Primary"); err != nil {
			return err
		}
		if err := validateGUCValue(ctx, primaryClient, "synchronous_standby_names", "", "Primary"); err != nil {
			return err
		}
	}
	...
	if setup.StandbyMultipooler != nil {
		standbyClient, err := endtoend.NewMultiPoolerTestClient(ctx, fmt.Sprintf("localhost:%d", setup.StandbyMultipooler.GrpcPort))
		if err != nil {
		...
		// Verify standby is in recovery mode
		inRecovery, err := queryStringValue(ctx, standbyClient, "SELECT pg_is_in_recovery()")
		if err != nil {
		...
		if err := validateGUCValue(ctx, standbyClient, "primary_conninfo", "", "Standby"); err != nil {
			return err
		}

		// Verify WAL replay not paused
		isPaused, err := queryStringValue(ctx, standbyClient, "SELECT pg_is_wal_replay_paused()")
		if err != nil {
...
func setupPoolerTest(t *testing.T, setup *MultipoolerTestSetup, opts ...cleanupOption) {
	...
	if err := validateCleanState(t.Context(), setup); err != nil {
	...
	if setup != nil && setup.PrimaryMultipooler != nil {
		...
		_, err = primaryPoolerClient.ExecuteQuery(t.Context(), "SELECT 1", 0)
	...
	if setup != nil && setup.StandbyMultipooler != nil {
		...
		_, err = standbyPoolerClient.ExecuteQuery(t.Context(), "SELECT 1", 0)
		require.NoError(t, err, "Failed to query primary pooler")
	}
	...
	if len(config.gucsToReset) > 0 {
		if primaryPoolerClient != nil {
			savedPrimaryGucs = saveGUCs(t, primaryPoolerClient, config.gucsToReset)
		}
		if standbyPoolerClient != nil {
			savedStandbyGucs = saveGUCs(t, standbyPoolerClient, config.gucsToReset)
		}
	}
	...
	if shouldConfigureReplication {
		if setup == nil || setup.StandbyMultipooler == nil || setup.PrimaryPgctld == nil {
			...
			if primaryPoolerClient != nil {
				_, err := primaryPoolerClient.ExecuteQuery(t.Context(), "ALTER SYSTEM SET synchronous_standby_names = ''", 0)
				if err == nil {
					_, err = primaryPoolerClient.ExecuteQuery(t.Context(), "SELECT pg_reload_conf()", 0)
					if err == nil {
			...
			ctxSetPrimary, cancelSetPrimary := context.WithTimeout(t.Context(), 5*time.Second)
			...
			if err != nil {
				...
				require.Eventually(t, func() bool {
					resp, err := standbyPoolerClient.ExecuteQuery(t.Context(), "SELECT status FROM pg_stat_wal_receiver", 1)
				...
				if config.pauseReplication {
					...
					_, err = standbyPoolerClient.ExecuteQuery(t.Context(), "SELECT pg_wal_replay_pause()", 1)
					if err != nil {
	...
	t.Cleanup(func() {
		...
		if len(savedPrimaryGucs) > 0 && primaryPoolerClient != nil {
			restoreGUCs(t, cleanupCtx, primaryPoolerClient, savedPrimaryGucs, "primary")
		}

		if len(savedStandbyGucs) > 0 && standbyPoolerClient != nil {
			restoreGUCs(t, cleanupCtx, standbyPoolerClient, savedStandbyGucs, "standby")
		}
		...
		if standbyPoolerClient != nil {
			_, err := standbyPoolerClient.ExecuteQuery(cleanupCtx, "SELECT pg_wal_replay_resume()", 1)
			if err != nil {
		...
		if len(config.tablesToDrop) > 0 && primaryPoolerClient != nil {
			for _, table := range config.tablesToDrop {
				_, err := primaryPoolerClient.ExecuteQuery(cleanupCtx, fmt.Sprintf("DROP TABLE IF EXISTS %s", table), 1)
				if err != nil {
		...
		require.Eventually(t, func() bool {
			return validateCleanState(cleanupCtx, setup) == nil
