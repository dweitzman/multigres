...
func TestDeadPrimaryRecovery(t *testing.T) {
	...
	t.Run("verify sync replication configured on final primary", func(t *testing.T) {
		...
		// Assert the real cohort is configured, not merely non-empty. A bare
		// NotEmpty would also pass on the fail-closed fence sentinel (which means
		// the primary is blocking writes), so check every cohort member's
		// application_name ("{cell}_{name}") is present. This proves the cohort was
		// reconciled, not left fenced.
		require.NotContains(t, syncStandbyNames, "fence",
			"final primary should hold the real cohort, not the fence sentinel")
