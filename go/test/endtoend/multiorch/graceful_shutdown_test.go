...
//   - it announces LIFECYCLE_SIGNAL_SHUTTING_DOWN to multiorch,
...
//     in-flight queries) → SHUTTING_DOWN announcement → pool Close →
//     pgctld.Stop(fast) → STOPPED announcement → process exit.
//  3. multiorch observes LIFECYCLE_SIGNAL_STOPPED on the cached
//     PoolerHealthState (from the snapshot or, if delivery races the EOF,
//     from the synthesized post-EOF STOPPED).
//  4. LeaderNeedsReplacement returns true → LeaderIsDownAnalyzer triggers
...
func TestPrimaryGracefulShutdownTriggersFailover(t *testing.T) {
	...
	// and seen a clean cluster state. Without this, the SHUTTING_DOWN
	// announcement we make below can race a stream that's still being
	// established, and multiorch never receives the signal.
