...
func waitForShardPrimary(t *testing.T, setup *shardsetup.ShardSetup, timeout time.Duration) string {
	...
	for time.Now().Before(deadline) {
		for name, inst := range setup.Multipoolers {
			...
			if status.Status.IsInitialized && status.Status.PoolerType == clustermetadatapb.PoolerType_POOLER_TYPE_PRIMARY {
...
func waitForStandbysInitialized(t *testing.T, setup *shardsetup.ShardSetup, expectedCount int, timeout time.Duration) {
	...
	for time.Now().Before(deadline) {
		...
		for name, inst := range setup.Multipoolers {
			...
			if status.Status.IsInitialized && status.Status.PoolerType == clustermetadatapb.PoolerType_POOLER_TYPE_REPLICA && status.Status.PostgresRunning {
				standbyCount++
			}
		}
