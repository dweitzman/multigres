...
`), 0o644)
	...
	t.Run("basic_commands_with_real_postgresql", func(t *testing.T) {
		...
		initCmd := utils.CommandContext(t, t.Context(), "", "pgctld", "init", "--pooler-dir", dataDir, "--config-file", pgctldConfigFile)
		...
		statusCmd := utils.CommandContext(t, t.Context(), "", "pgctld", "status", "--pooler-dir", dataDir, "--config-file", pgctldConfigFile)
		...
		helpCmd := utils.CommandContext(t, t.Context(), "", "pgctld", "--help")
		...
		versionCmd := utils.CommandContext(t, t.Context(), "", "postgres", "--version")
		...
		initdbCmd := utils.CommandContext(t, t.Context(), "", "initdb", "--help")
...
`), 0o644)
	...
	t.Run("startup_performance", func(t *testing.T) {
		...
		startTime := time.Now()
		initCmd := utils.CommandContext(t, t.Context(), tempDir, "pgctld", "init", "--pooler-dir", dataDir, "--pg-port", strconv.Itoa(perfTestPort), "--config-file", pgctldConfigFile)
		setupTestEnv(initCmd)
		...
		startCmd := utils.CommandContext(t, t.Context(), tempDir, "pgctld", "start", "--pooler-dir", dataDir, "--pg-port", strconv.Itoa(perfTestPort), "--config-file", pgctldConfigFile)
		...
		// Clean shutdown
		stopCmd := utils.CommandContext(t, t.Context(), tempDir, "pgctld", "stop", "--pooler-dir", dataDir, "--pg-port", strconv.Itoa(perfTestPort), "--config-file", pgctldConfigFile)
	...
	t.Run("multiple_rapid_operations", func(t *testing.T) {
		...
		for i := range 3 {
			...
			// Stop
			stopCmd := utils.CommandContext(t, t.Context(), tempDir, "pgctld", "stop", "--pooler-dir", dataDir, "--pg-port", strconv.Itoa(perfTestPort), "--mode", "fast", "--config-file", pgctldConfigFile)
			setupTestEnv(stopCmd)
			err = stopCmd.Run()
			require.NoError(t, err)
...
`), 0o644)
	...
	t.Run("version_compatibility", func(t *testing.T) {
		...
		versionCmd := utils.CommandContext(t, t.Context(), tempDir, "postgres", "--version")
		output, err := versionCmd.Output()
		require.NoError(t, err)
		t.Logf("PostgreSQL version: %s", string(output))

		initCmd := utils.CommandContext(t, t.Context(), tempDir, "pgctld", "init", "--pooler-dir", dataDir, "--config-file", pgctldConfigFile, "--pg-port", strconv.Itoa(testPort))
		setupTestEnv(initCmd)
		initOutput, err := initCmd.CombinedOutput()
		if err != nil {
			t.Logf("pgctld init failed with output: %s", string(initOutput))
		...
		startCmd := utils.CommandContext(t, t.Context(), tempDir, "pgctld", "start", "--pooler-dir", dataDir, "--pg-port", strconv.Itoa(testPort), "--config-file", pgctldConfigFile)
		setupTestEnv(startCmd)
		startOutput, err := startCmd.CombinedOutput()
		if err != nil {
			t.Logf("pgctld start failed with output: %s", string(startOutput))
		...
		statusCmd := utils.CommandContext(t, t.Context(), tempDir, "pgctld", "status", "--pooler-dir", dataDir, "--pg-port", strconv.Itoa(testPort), "--config-file", pgctldConfigFile)
		setupTestEnv(statusCmd)
		...
		// Clean shutdown
		stopCmd := utils.CommandContext(t, t.Context(), tempDir, "pgctld", "stop", "--pooler-dir", dataDir, "--pg-port", strconv.Itoa(testPort), "--config-file", pgctldConfigFile)
		setupTestEnv(stopCmd)
		err = stopCmd.Run()
		require.NoError(t, err)
	})
...
`), 0o644)
	...
	// Initialize database
	initCmd := utils.CommandContext(t, t.Context(), tempDir, "pgctld", "init", "--pooler-dir", dataDir, "--pg-port", strconv.Itoa(testPort), "--config-file", pgctldConfigFile)
	setupTestEnv(initCmd)
	initOutput, err := initCmd.CombinedOutput()
	if err != nil {
		t.Logf("pgctld init failed with output: %s", string(initOutput))
	...
	// Restart as standby
	restartCmd := utils.CommandContext(t, t.Context(), tempDir, "pgctld", "restart", "--pooler-dir", dataDir, "--pg-port", strconv.Itoa(testPort), "--as-standby", "--config-file", pgctldConfigFile)
	...
	recoveryCheckCmd := utils.CommandContext(t, t.Context(), tempDir, "psql",
		"-h", socketDir,
...
func TestPostgreSQLAuthentication(t *testing.T) {
	...
	t.Run("pgpassword_authentication", func(t *testing.T) {
		...
		initCmd := utils.CommandContext(t, t.Context(), baseDir, "pgctld", "init", "--pooler-dir", baseDir, "--pg-port", strconv.Itoa(port))
		...
		startCmd := utils.CommandContext(t, t.Context(), baseDir, "pgctld", "start", "--pooler-dir", baseDir, "--pg-port", strconv.Itoa(port))
		startCmd.Env = append(startCmd.Env,
			"PGCONNECT_TIMEOUT=5",
			fmt.Sprintf("PGPASSWORD=%s", testPassword),
		)
		...
		socketCmd := utils.CommandContext(t, t.Context(), baseDir, "psql",
			"-h", socketDir,
		...
		statusCmd := utils.CommandContext(t, t.Context(), baseDir, "pgctld", "status", "--pooler-dir", baseDir)
		...
		tcpCmd := utils.CommandContext(t, t.Context(), baseDir, "psql",
			"-h", "localhost",
		...
		tcpCmd.Env = append(tcpCmd.Env, fmt.Sprintf("PGPASSWORD=%s", testPassword))
		...
		wrongPasswordCmd := utils.CommandContext(t, t.Context(), baseDir, "psql",
			"-h", "localhost",
			"-p", strconv.Itoa(port),
			"-U", "postgres",
			"-d", "postgres",
		...
		wrongPasswordCmd.Env = append(wrongPasswordCmd.Env, "PGPASSWORD=wrong_password")
		...
		roleCheckCmd := utils.CommandContext(t, t.Context(), baseDir, "psql",
			"-h", socketDir,
			"-p", strconv.Itoa(port),
			"-U", "postgres",
			"-d", "postgres",
		...
		dbCheckCmd := utils.CommandContext(t, t.Context(), baseDir, "psql",
			"-h", socketDir,
			"-p", strconv.Itoa(port),
			"-U", "postgres",
			"-d", "postgres",
		...
		privilegeCheckCmd := utils.CommandContext(t, t.Context(), baseDir, "psql",
			"-h", socketDir,
			"-p", strconv.Itoa(port),
			"-U", "postgres",
			"-d", "postgres",
		...
		stopCmd := utils.CommandContext(t, t.Context(), baseDir, "pgctld", "stop", "--pooler-dir", baseDir)
		stopCmd.Env = append(stopCmd.Env, "PGCONNECT_TIMEOUT=5")
		err = stopCmd.Run()
	...
	t.Run("password_file_authentication", func(t *testing.T) {
		...
		initCmd := utils.CommandContext(t, t.Context(), baseDir, "pgctld", "init", "--pooler-dir", baseDir, "--pg-pwfile", pwfile, "--pg-port", strconv.Itoa(port))
		initCmd.Env = append(initCmd.Env, "PGCONNECT_TIMEOUT=5")
		output, err := initCmd.CombinedOutput()
		require.NoError(t, err, "pgctld init should succeed, output: %s", string(output))
		...
		t.Logf("Starting PostgreSQL server")
		startCmd := utils.CommandContext(t, t.Context(), baseDir, "pgctld", "start", "--pooler-dir", baseDir, "--pg-port", strconv.Itoa(port))
		startCmd.Env = append(startCmd.Env, "PGCONNECT_TIMEOUT=5")
		output, err = startCmd.CombinedOutput()
		require.NoError(t, err, "pgctld start should succeed, output: %s", string(output))
	...
	t.Run("password_source_conflict", func(t *testing.T) {
		...
		initCmd := utils.CommandContext(t, t.Context(), baseDir, "pgctld", "init", "--pooler-dir", baseDir, "--pg-pwfile", pwfile)
		initCmd.Env = append(initCmd.Env,
			"PGCONNECT_TIMEOUT=5",
...
`), 0o644)
	...
	t.Run("complete_lifecycle_via_cli", func(t *testing.T) {
		// Step 1: Initialize the database first
		initCmd := utils.CommandContext(t, t.Context(), tempDir, "pgctld", "init", "--pooler-dir", dataDir, "--config-file", pgctldConfigFile)
		initCmd.Env = append(initCmd.Env,
		...
		statusCmd := utils.CommandContext(t, t.Context(), tempDir, "pgctld", "status", "--pooler-dir", dataDir, "--config-file", pgctldConfigFile)
		statusCmd.Env = append(statusCmd.Env,
			"PATH="+filepath.Join(tempDir, "bin")+":"+os.Getenv("PATH"))
		output, err := statusCmd.CombinedOutput()
		if err != nil {
		...
		startCmd := utils.CommandContext(t, t.Context(), tempDir, "pgctld", "start", "--pooler-dir", dataDir, "--config-file", pgctldConfigFile)
		startCmd.Env = append(startCmd.Env,
			"PATH="+filepath.Join(tempDir, "bin")+":"+os.Getenv("PATH"))
		err = startCmd.Run()
		require.NoError(t, err)
		...
		reloadCmd := utils.CommandContext(t, t.Context(), tempDir, "pgctld", "reload-config", "--pooler-dir", dataDir, "--config-file", pgctldConfigFile)
		reloadCmd.Env = append(reloadCmd.Env,
			"PATH="+filepath.Join(tempDir, "bin")+":"+os.Getenv("PATH"))
		err = reloadCmd.Run()
		require.NoError(t, err)

		// Step 6: Restart PostgreSQL
		restartCmd := utils.CommandContext(t, t.Context(), tempDir, "pgctld", "restart", "--pooler-dir", dataDir, "--config-file", pgctldConfigFile)
		...
		stopCmd := utils.CommandContext(t, t.Context(), tempDir, "pgctld", "stop", "--pooler-dir", dataDir, "--config-file", pgctldConfigFile)
		stopCmd.Env = append(stopCmd.Env,
			"PATH="+filepath.Join(tempDir, "bin")+":"+os.Getenv("PATH"))
		err = stopCmd.Run()
		require.NoError(t, err)
...
`), 0o644)
	...
	for i := 0; i < 3; i++ {
		t.Run(fmt.Sprintf("cycle_%d", i+1), func(t *testing.T) {
			...
			// Stop PostgreSQL
			stopCmd := utils.CommandContext(t, t.Context(), tempDir, "pgctld", "stop", "--pooler-dir", dataDir, "--mode", "fast", "--config-file", pgctldConfigFile)
			stopCmd.Env = append(stopCmd.Env,
				"PATH="+filepath.Join(tempDir, "bin")+":"+os.Getenv("PATH"))
			err = stopCmd.Run()
			require.NoError(t, err)
...
`), 0o644)
	...
	t.Run("start_with_nonexistent_data_dir", func(t *testing.T) {
		...
		startCmd := utils.CommandContext(t, t.Context(), tempDir, "pgctld", "start", "--pooler-dir", nonexistentDir, "--config-file", pgctldConfigFile)
		startCmd.Env = append(startCmd.Env,
			"PATH="+filepath.Join(tempDir, "bin")+":"+os.Getenv("PATH"))
		err := startCmd.Run()
		...
		initCmd := utils.CommandContext(t, t.Context(), tempDir, "pgctld", "init", "--pooler-dir", nonexistentDir, "--config-file", pgctldConfigFile)
		initCmd.Env = append(initCmd.Env,
			"PATH="+filepath.Join(tempDir, "bin")+":"+os.Getenv("PATH"))
		err = initCmd.Run()
		require.NoError(t, err)
		...
		// Clean stop
		stopCmd := utils.CommandContext(t, t.Context(), tempDir, "pgctld", "stop", "--pooler-dir", nonexistentDir, "--mode", "immediate", "--config-file", pgctldConfigFile)
		stopCmd.Env = append(stopCmd.Env,
			"PATH="+filepath.Join(tempDir, "bin")+":"+os.Getenv("PATH"))
		_ = stopCmd.Run()
	})
	...
	t.Run("double_start_attempt", func(t *testing.T) {
		...
		startCmd2 := utils.CommandContext(t, t.Context(), tempDir, "pgctld", "start", "--pooler-dir", dataDir, "--config-file", pgctldConfigFile)
		startCmd2.Env = append(startCmd2.Env,
			"PATH="+filepath.Join(tempDir, "bin")+":"+os.Getenv("PATH"))
		...
		// Clean stop
		stopCmd := utils.CommandContext(t, t.Context(), tempDir, "pgctld", "stop", "--pooler-dir", dataDir, "--mode", "immediate", "--config-file", pgctldConfigFile)
		stopCmd.Env = append(stopCmd.Env,
			"PATH="+filepath.Join(tempDir, "bin")+":"+os.Getenv("PATH"))
		_ = stopCmd.Run()
	})
...
func TestOrphanDetectionWithRealPostgreSQL(t *testing.T) {
	...
	initCmd := utils.CommandContext(t, t.Context(), tempDir, "pgctld", "init", "--pooler-dir", dataDir, "--pg-port", strconv.Itoa(pgPort), "--config-file", pgctldConfigFile)
	setupTestEnv(initCmd)
