...
// UpdateInterval updates the interval for future callback executions.
// If the runner is currently running, the new interval takes effect after
// the current callback completes. If stopped, the interval will be used
// when Start() is next called.
func (r *PeriodicRunner) UpdateInterval(newInterval time.Duration) {
	r.mu.Lock()
	defer r.mu.Unlock()
