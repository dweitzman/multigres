...
// Example demonstrates basic retry usage with exponential backoff and full jitter.
func Example() {
	...
	err := r.Do(ctx, func(attempt int) error {
		// Your operation here
		// Return error to retry, nil for success
		return nil
	})
	if err != nil {
		// Handle final error
...
// Example_nonRetryableError shows how to stop retries on permanent failures.
func Example_nonRetryableError() {
	r := New(100*time.Millisecond, 5*time.Second)
	ctx := context.Background()

	err := r.Do(ctx, func(attempt int) error {
		// Simulate checking for auth errors
		if attempt == 0 {
			// Permanent error - stop immediately
			return NonRetryableError(errors.New("unauthorized"))
		}
		return nil
	})
	if err != nil {
		// err will be the unwrapped error: "unauthorized"
	}
}
